...
const Calendar = {
  ...
  async applySameAsLastWeek() {
    var btn = document.getElementById('btn-same-as-last-week');
    if (!btn) return;
    btn.disabled    = true;
    btn.textContent = '取得中…';

    try {
      var periodsResult = await API.getPeriods();
      if (!periodsResult.ok || !periodsResult.periods) {
        throw new Error('期間情報の取得に失敗しました');
      }

      var allPeriods = periodsResult.periods;
      var currentIdx = allPeriods.findIndex(function(p) {
        return p.id === this.currentPeriod.id;
      }.bind(this));
      ...
      var prevPeriod   = allPeriods[currentIdx - 1];
      var shiftsResult = await API.getMyShifts(AppState.userId, prevPeriod.id);
      ...
      // 曜日 → 内部モデルのマップを作成（前期間データはGAS形式 {date,available,start,end}）
      var dowMap = {};
      shiftsResult.shifts.forEach(function(s) {
        var dow = new Date(s.date + 'T00:00:00').getDay();
        if (!s.available) {
          dowMap[dow] = {
            available  : false,
            lunch      : false,
      ...
      var dates        = this._getDatesInRange(this.currentPeriod.start, this.currentPeriod.end);
      var appliedCount = 0;
      ...
      dates.forEach(function(date) {
        ...
        if (prev.available) {
          ...
          this.shifts[date] = {
            ...
            lunch      : prev.lunch,
            dinner     : prev.dinner,
            dinnerStart: prev.dinnerStart,
            dinnerEnd  : prev.dinnerEnd
          };

          // ランチ/ディナーボタンのUI更新
          var lunchBtn  = document.getElementById('lunch-btn-'  + date);
          var dinnerBtn = document.getElementById('dinner-btn-' + date);
          if (lunchBtn)  lunchBtn.classList.toggle('active',  prev.lunch);
          if (dinnerBtn) dinnerBtn.classList.toggle('active', prev.dinner);

          // ディナー詳細セクションの表示切り替え
          var dinnerDetail = document.getElementById('dinner-detail-' + date);
          if (dinnerDetail) dinnerDetail.style.display = prev.dinner ? '' : 'none';

          if (prev.dinner) {
            var startEl = document.getElementById('dinner-start-' + date);
            var endEl   = document.getElementById('dinner-end-'   + date);
            if (startEl) startEl.value = prev.dinnerStart;
            if (endEl)   endEl.value   = prev.dinnerEnd;

            var hint = document.getElementById('dinner-hint-' + date);
            if (hint) hint.textContent = prev.dinnerStart + '〜' + prev.dinnerEnd;
          }
        }
        appliedCount++;
      }, this);
